"""
OpenAI-based Intelligent Tagging Service (feature-scoped)

Moved under infrastructure/tagging/services to align with feature-first layout.
"""
...
"""
    ...
    async def categorize_tags(self, tags: List[str]) -> Dict[str, List[str]]:
        """Categorize tags into semantic groups using OpenAI"""
        try:
            system_prompt = (
    ...
    async def find_similar_tags(self, tag: str, limit: int = 10) -> List[Dict[str, Any]]:
...
"""
    ...
    async def extract_tags_from_breathing_session(self, pattern_name: str, duration_minutes: int, effectiveness_rating: Optional[int] = None, notes: Optional[str] = None, user_context: Optional[Dict[str, Any]] = None) -> TagExtractionResult:
        try:
            analysis_content = f"""
