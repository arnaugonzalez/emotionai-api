...
def test_chat_response_defaults_crisis_factory_and_model_dump():
...
def test_additional_dtos_validation_and_model_dump():
    uid = uuid4()

    status = AgentStatusResponse(active=True, agent_type="therapy")
    assert status.model_dump()["active"] is True
    ...
    assert token.model_dump()["token_type"] == "bearer"
